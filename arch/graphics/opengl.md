...
Development of OpenGL ceased in 2017 in favour of [Vulkan](Vulkan "wikilink"), the \"next generation\" API which offers
higher performance on newer hardware.

## Installation

To run applications that use OpenGL, you will need to [install](install "wikilink") the correct driver(s) for your
...
[AMD](AMD "wikilink") (formerly [ATI](ATI "wikilink")) and [NVIDIA](NVIDIA "wikilink") GPUs. Mesa also provides software
[rasterizers](Wikipedia:Rasterisation "wikilink"), such as llvmpipe.
...
-   ```{=mediawiki}
    ...
    -   ```{=mediawiki}
        {{ic|nouveau}}
        ```
        : [Nouveau](Nouveau "wikilink") is the open-source driver for NVIDIA GPUs.

    -   ```{=mediawiki}
        {{ic|virtio_gpu}}
        ```
        : a virtual GPU driver for virtio, can be used with [QEMU](QEMU "wikilink") based VMMs (like
        [KVM](KVM "wikilink") or [Xen](Xen "wikilink")).

    -   ```{=mediawiki}
        {{ic|vmwgfx}}
        ```
        : for [VMware](VMware "wikilink") virtual GPUs.
    ...
    -   ```{=mediawiki}
        {{ic|zink}}
        ```
        : a Gallium driver used to run OpenGL on top of [Vulkan](Vulkan "wikilink").

    -   ```{=mediawiki}
        {{ic|d3d12}}
        ```
        : for OpenGL 3.3 support on devices that only support D3D12 (i.e. [WSL](WSL "wikilink")).
...
-   ```{=mediawiki}
    {{Pkg|nvidia-utils}}
    ```
    is the proprietary driver for [NVIDIA](NVIDIA "wikilink") GPUs, which includes an OpenGL implementation.
...
For [Hybrid graphics](Hybrid_graphics "wikilink"), see [PRIME](PRIME "wikilink").
