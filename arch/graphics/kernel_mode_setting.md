[de:KMS](de:KMS "wikilink") [es:Kernel mode setting](es:Kernel_mode_setting "wikilink") [fr:Kernel mode
setting](fr:Kernel_mode_setting "wikilink") [ja:カーネルモード設定](ja:カーネルモード設定 "wikilink") [ru:Kernel mode
setting](ru:Kernel_mode_setting "wikilink") [zh-hans:Kernel mode setting](zh-hans:Kernel_mode_setting "wikilink")
`{{Related articles start}}`{=mediawiki} `{{Related|ATI}}`{=mediawiki} `{{Related|Intel graphics}}`{=mediawiki}
`{{Related|Nouveau}}`{=mediawiki} `{{Related articles end}}`{=mediawiki}

```{=mediawiki}
{{Expansion|KMS and rootless X (1.16), see [[Talk:Kernel mode setting]] and [[Xorg#Rootless Xorg]].}}
```
Kernel [Mode Setting](Wikipedia:Mode-setting "wikilink") (KMS) is a method for setting display resolution and depth in
the kernel space rather than user space.
...
[DRI2](wikipedia:Direct_Rendering_Infrastructure#DRI2 "wikilink")) which will help reduce artifacts and increase 3D
performance, even kernel space power-saving.
...
fancy graphics in [virtual consoles](virtual_console "wikilink"). Also, each time a switch from X to a virtual console
was made (`{{ic|Ctrl+Alt+F2}}`{=mediawiki}), the server had to give control over the video card to the kernel, which was
slow and caused flickering. The same \"painful\" process happened when the control was given back to the X server
(`{{ic|Alt+F7}}`{=mediawiki} when X runs in VT7).
...
-   Any other framebuffer drivers (such as [uvesafb](uvesafb "wikilink")).
...
[Intel](Intel "wikilink"), [Nouveau](Nouveau "wikilink"), [ATI](ATI "wikilink") and [AMDGPU](AMDGPU "wikilink") drivers
already enable KMS automatically for all chipsets, so you do not need to do anything.

The proprietary [NVIDIA](NVIDIA "wikilink") driver supports KMS (since 364.12), which has to be [manually
enabled](NVIDIA#DRM_kernel_mode_setting "wikilink").
...
-   ```{=mediawiki}
    {{ic|amdgpu}}
    ```
    for [AMDGPU](AMDGPU "wikilink"), or `{{ic|radeon}}`{=mediawiki} when using the legacy [ATI](ATI "wikilink") driver.

-   ```{=mediawiki}
    {{ic|i915}}
    ```
    for [Intel graphics](Intel_graphics "wikilink").

-   ```{=mediawiki}
    {{ic|nouveau}}
    ```
    for the open-source [Nouveau](Nouveau "wikilink") driver.

-   ```{=mediawiki}
    {{ic|nvidia nvidia_modeset nvidia_uvm nvidia_drm}}
    ```
    for the out-of-tree `{{Pkg|nvidia}}`{=mediawiki} and `{{Pkg|nvidia-open}}`{=mediawiki} drivers. See [NVIDIA#DRM
    kernel mode setting](NVIDIA#DRM_kernel_mode_setting "wikilink") for details.
...
-   Depending on [QEMU](QEMU "wikilink") graphics in use (*qemu* option `{{ic|-vga ''type''}}`{=mediawiki} or
    [libvirt](libvirt "wikilink")
...
-   Depending on [VirtualBox](VirtualBox "wikilink") graphics controller:
    -   ```{=mediawiki}
        {{ic|vmwgfx}}
        ```
        for VMSVGA,
...
file into [initramfs](initramfs "wikilink") as well:
...
```
Then [regenerate the initramfs](regenerate_the_initramfs "wikilink").

#### Booster

If you use [Booster](Booster "wikilink"), you can load required modules with this config change:
...
```
If you are using the [#Forcing modes and EDID](#Forcing_modes_and_EDID "wikilink") method, you should embed the custom
file into your booster images as well:
...
```
Then [regenerate the booster images](Booster#Usage "wikilink").

#### Dracut

[Dracut](Dracut "wikilink") enables early loading (at the initramfs stage, via `{{ic|modprobe}}`{=mediawiki}) through
its `{{ic|--force_drivers}}`{=mediawiki} command or `{{ic|force_drivers+{{=}}`{=mediawiki}\"\"}} config entry line. For
example:
...
See [Linux console#Fonts](Linux_console#Fonts "wikilink") for how to change your console font to a large font. The
Terminus font (`{{Pkg|terminus-font}}`{=mediawiki}) is available in many sizes, such as `{{ic|ter-132b}}`{=mediawiki}
which is larger.
...
none or just a skewed [EDID](Wikipedia:EDID "wikilink") file. The kernel will try to catch this case and will set one of
the most typical resolutions.
...
To load it at boot, specify the following in the [kernel command line](kernel_command_line "wikilink"):
...
If you are doing [early KMS](#Early_KMS_start "wikilink"), you must [include the custom EDID file in the
initramfs](Mkinitcpio#BINARIES_and_FILES "wikilink"), otherwise you will run into problems.
...
[lockdown mode](Security#Kernel_lockdown_mode "wikilink"). This is very useful if you swap the monitors on a connector
or just for testing. Once you have an EDID file as above, run:
...
parameter. See [Kernel parameters](Kernel_parameters "wikilink") for more info.
...
```
[Category:Graphics](Category:Graphics "wikilink") [Category:Kernel](Category:Kernel "wikilink")
