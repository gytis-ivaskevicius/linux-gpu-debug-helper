[ja:ブラウザ拡張機能](ja:ブラウザ拡張機能 "ja:ブラウザ拡張機能"){.wikilink} [ru:Browser
extensions](ru:Browser_extensions "ru:Browser extensions"){.wikilink}
[zh-hans:浏览器扩展](zh-hans:浏览器扩展 "zh-hans:浏览器扩展"){.wikilink}

This article lists some [browser extensions](Wikipedia:Browser_extension "browser extensions"){.wikilink} available for
[Firefox](Firefox "Firefox"){.wikilink} and/or [Chromium](Chromium "Chromium"){.wikilink}.
...
To simplify maintenance, this article does not link store pages or [AUR](AUR "AUR"){.wikilink} packages of extensions.
Readers are advised to obtain extensions through the linked official websites if no package is available.

## Privacy

See also [Firefox/Privacy](Firefox/Privacy "Firefox/Privacy"){.wikilink} and [Chromium/Tips and
tricks#Security](Chromium/Tips_and_tricks#Security "Chromium/Tips and tricks#Security"){.wikilink}.
...
Websites can be augmented using user style sheets and JavaScript
[userscripts](Wikipedia:userscript "userscripts"){.wikilink}.
...
There are various extensions providing [vi](vi "vi"){.wikilink}-style keyboard shortcuts.
...
Extensions to edit `{{ic|<textarea>}}`{=mediawiki}s with native [text editors](text_editor "text editor"){.wikilink}:
...
On modern browsers, [Ruffle](https://ruffle.rs/) is a Flash Player emulator written in [Rust](Rust "Rust"){.wikilink},
available through the use of [WebAssembly](Wikipedia:WebAssembly "WebAssembly"){.wikilink}. Unlike the defunct Flash
Player plugin and Lightspark, Ruffle is only available in browsers as an extension (e.g. for
...
To run natively, Ruffle can be [installed](install "install"){.wikilink} with one of the
`{{AUR|ruffle-git}}`{=mediawiki} or `{{AUR|ruffle-nightly-bin}}`{=mediawiki} packages.

## See also {#see_also}

- [Firefox add-ons sorted by user counts](https://addons.mozilla.org/firefox/search/?sort=users)
- [List of Firefox extensions at
  Wikipedia](Wikipedia:List_of_Firefox_extensions "List of Firefox extensions at Wikipedia"){.wikilink}

[Category:Web browser](Category:Web_browser "Category:Web browser"){.wikilink} [Category:Lists of
