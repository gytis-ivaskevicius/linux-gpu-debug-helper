[ja:東方Project](ja:東方Project "ja:東方Project"){.wikilink}
[zh-hans:Touhou](zh-hans:Touhou "zh-hans:Touhou"){.wikilink} [Touhou
Project](Wikipedia:Touhou_Project "Touhou Project"){.wikilink} is the name of a series of *danmaku* games, also known as
[bullet-hell](Wikipedia:Bullet_hell "bullet-hell"){.wikilink} shooters.

Bullet-hell shooters is a genre of [2D](Wikipedia:2D_computer_graphics "2D"){.wikilink} shooters based on really complex
...
[manga](Wikipedia:Manga "manga"){.wikilink} (Japanese comics) and even unofficial games.
...
[PC-98](Wikipedia:PC-98 "PC-98"){.wikilink} games can be played using Linux-native X Neko Project II
emulator---[install](install "install"){.wikilink} the `{{AUR|xnp2}}`{=mediawiki} package.

The following packages only depend on [Wine](Wine "Wine"){.wikilink} to run, and
[Timidity++](Timidity++ "Timidity++"){.wikilink} to play [MIDI](Wikipedia:MIDI "MIDI"){.wikilink} music. They install
the free trial versions. You can easily replace the trials with the full game if you have it.
...
We need help [packaging](Wine_PKGBUILD_Guidelines "packaging"){.wikilink} more Touhou games for the [Arch User
Repository](Arch_User_Repository "Arch User Repository"){.wikilink}. This is a list of games that have free,
downloadable trial editions to build off of:
...
[Python](Wikipedia:Python_(programming_language) "Python"){.wikilink} engine to make the games more portable. See the
...
```
Remember to [start](start "start"){.wikilink} the `{{ic|timidity.service}}`{=mediawiki} user unit before playing.
...
self-updating, multilingual translation of the [Touhou Project](wikipedia:Touhou_Project "Touhou Project"){.wikilink}
games on [Touhou Patch Center](https://thpatch.net/), but can theoretically be used for just about any other patch for
these games, without going through that site.
...
- [Install](Install "Install"){.wikilink} `{{AUR|thcrap-steam-proton-wrapper-git}}`{=mediawiki}. For
  [Flatpak](Flatpak "Flatpak"){.wikilink} version of Steam install
  `{{ic|com.valvesoftware.Steam.Utility.thcrap_steam_proton_wrapper}}`{=mediawiki} from *Flathub* instead.
- Change your Touhou game launch options. Right click your Touhou games in your Steam library, then click *Properties*.
  Under the *General* tab, change *LAUNCH OPTIONS* to
...
- Copy `{{ic|vpatch.exe}}`{=mediawiki}, `{{ic|vpatch.ini}}`{=mediawiki} and `{{ic|vpatch_thxx.dll}}`{=mediawiki} to your
  game directory `{{ic|~/.local/share/Steam/steamapps/common/thxx/}}`{=mediawiki} (or
...
- Replace the `{{ic|~/.local/share/Steam/steamapps/common/thxx/thxx.exe}}`{=mediawiki} with the one you legally obtained
  from original disk.
...
- [Wine PKGBUILD Guidelines](Wine_PKGBUILD_Guidelines "Wine PKGBUILD Guidelines"){.wikilink}
