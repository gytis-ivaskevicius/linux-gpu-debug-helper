...
All official Hypr\* programs are listed in the project\'s [wiki page](https://wiki.hypr.land/Hypr-Ecosystem/) along with
documentation.
...
does not advertise support for one. Though one can start hyprland directly from tty with `start-hyprland` or with
...
`$XDG_CONFIG_HOME/hypr/hyprland.lua` if it does not exist. An example configuration can be found in the [project\'s git
repository](https://github.com/hyprwm/Hyprland/blob/v0.55.2-b/example/hyprland.lua).
...
wayland.windowManager.hyprland](https://nix-community.github.io/home-manager/options.xhtml#opt-wayland.windowManager.hyprland.enable).
...
### Using symlinks and Home Manager {#using_symlinks_and_home_manager}
